...
{
    if (!require_sender_is_member(client))
        return;
...
void Channel::mode_o(const Command &cmd, Client &sender, bool valid, Client &target)
{
    if (!require_operator(sender))
        return;
    if (!require_sender_is_member(sender))
        return;
    if (!require_target_is_member(sender, target))
        return;
    reply_cmd_all(sender, cmd);
    if (valid)
    {
...
void Channel::mode_l_add(const Command &cmd, Client &sender, const std::string &limit_num_str)
{
    if (!require_operator(sender))
        return;
    if (!require_valid_num(sender, limit_num_str))
        return;
    has_limit_ = true;
